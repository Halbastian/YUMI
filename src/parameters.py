...
class Parameters():
    ...
    feasibilityObjectives = {
        'jointPositionBound': True,
        'JointVelocityBound': True,
        'elbowCollision': True,
        'gripperCollision': False,  # only used for individual control
        'jointPotential': False}  # TODO re-enable this (right now, it makes traj-ctrl very keen to stopping)
    ...
    gripperRightLocal = utils.FramePose(position=np.array([0, 0, 0.166]), quaternion=np.array([0, 0, 0, 1]))
    gripperLeftLocal = utils.FramePose(position=np.array([0, 0, 0.166]), quaternion=np.array([0, 0, 0, 1]))
