...
class YumiController(object):
    ...
    def inverseKinematics(self, action):
        ...
        if action['elbowCollision']:
            ...
            self.selfCollisionElbow.compute(jacobianRightElbow, jacobianLeftElbow, self.yumiElbowPoseR, self.yumiElbowPoseL)
        ...
        if action['controlSpace'] == 'individual':
            ...
            if 'relativeVelocity' in action:
                self.relativeControl.compute(
                    controlVelocity=action['relativeVelocity'],
            ...
            if 'absoluteVelocity' in action:
                self.absoluteControl.compute(
                    controlVelocity=action['absoluteVelocity'],
                    jacobian=self.jacobianCombined)
    ...
    def resetPose(self, resetPos=Parameters.resetPos):
        ...
        q, dq = utils.calcPosVel(self.startJointPosition, np.zeros(14), resetPos, np.zeros(14), self.finalTime, self.resetTime)
