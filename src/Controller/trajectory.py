...
class Trajectory(object):
    ...
    def updateTrajectory(self, trajcetoryParameters, velLeftInit, velRightInit):
        ...
        for i in range(0, self.numberOfTrajectoryParameters):
            tfMatrixRight = self.transformer.fromTranslationRotation(np.zeros(3), trajcetoryParameters[i].orientationRight)
            tfMatrixLeft = self.transformer.fromTranslationRotation(np.zeros(3), trajcetoryParameters[i].orientationLeft)
        ...
        for i in range(1, self.numberOfTrajectoryParameters-1):
            vel = self.calcPointVel(
                trajcetoryParameters[i-1].positionRight, trajcetoryParameters[i].positionRight, trajcetoryParameters[i+1].positionRight, 
                trajcetoryParameters[i].pointTime, trajcetoryParameters[i+1].pointTime)
            self.positionVelocitiesRight.append(vel)
            vel = self.calcPointVel(
                trajcetoryParameters[i-1].positionLeft, trajcetoryParameters[i].positionLeft, trajcetoryParameters[i+1].positionLeft, 
                trajcetoryParameters[i].pointTime, trajcetoryParameters[i+1].pointTime)
    ...
    def getTarget(self):
        ...
        q, dq = utils.calcPosVel(
            qi=self.trajectory[self.index-1].positionRight,
        ...
        q, dq = utils.calcPosVel(
            qi=self.trajectory[self.index-1].positionLeft,
        ...
        quat, we = self.calcOrientation(
            Ri=self.rotationMatrixRight[self.index-1],
            Rf=self.rotationMatrixRight[self.index])
        self.targetOrientation[0:4] = quat
        self.desiredVelocity[3:6] = we

        quat, we = self.calcOrientation(
            Ri=self.rotationMatrixLeft[self.index-1],
